...
{
    // To reset the camera ,to go to the player
    [SerializeField]
    ...
    void Start()
    {
        ...
        // Store the last target position by using the player postion
    ...
    void FixedUpdate()
    {
        if (followsPlayer)
        {
            ...
            // Here we mean with the comparison, the camera will follow the player when he goes right 
            if (aheadTargetPos.x >= transform.position.x)
            {
                // SmoothDamp will move from 'transform.postion' to 'aheadTargetPos' by refering to the velocity using the speed
